...
function getElapsedSeconds(day, activeTimer, now) {
  if (!activeTimer) return "";

  return Math.max(Math.floor((now.getTime() - activeTimer.startedAt) / 1000) - Number(day.breakMinutes || 0) * 60, 0);
}

function formatDuration(value) {
  const elapsedSeconds = Math.max(Number(value || 0), 0);
...
function formatHoursDuration(hours) {
  return formatDuration(Math.round(Number(hours || 0) * 3600));
}

function formatRunningTime(day, activeTimer, now) {
  return formatDuration(getElapsedSeconds(day, activeTimer, now));
}
...
function RollingNumber({ value, active = false }) {
  return String(value).split("").map((character, index) => (
    <span
      className={active ? "rolling-char active" : "rolling-char"}
      key={`${character}-${index}`}
    >
      {character}
    </span>
...
function App() {
  ...
  const activeWorkSeconds = isWorkTimerRunning ? getElapsedSeconds(currentWorkDay, activeTimer, now) : 0;
  const activeWorkHoursText = activeWorkHours.toFixed(isWorkTimerRunning ? 4 : 2);
  const activeWorkPayText = isWorkTimerRunning ? `$${activeWorkPay.toFixed(4)}` : money(activeWorkPay);
  const currentWorkDayCompleted = Boolean(currentWorkDay.start && currentWorkDay.end && !isWorkTimerRunning);
  const activeClockProgress = isWorkTimerRunning ? ((activeWorkSeconds % 3600) / 3600) * 100 : currentWorkDayCompleted ? 100 : 0;
  const clockSecondsForFace = isWorkTimerRunning ? activeWorkSeconds : Math.round(activeWorkHours * 3600);
  const clockSecondAngle = clockSecondsForFace * 6;
  const clockMinuteAngle = (clockSecondsForFace / 60) * 6;
        ...
        <div className="punch-control">
          ...
          {isRunning && (
            <span className="live-clock">
              <RollingNumber value={formatRunningTime(day, activeTimer, now)} active />
            </span>
          )}
        </div>
      ...
      <main className="app-shell setup-shell">
        <section className="setup-panel">
          ...
          <div className="setup-actions">
            ...
            <button className="primary-button" type="button" onClick={saveRateAndContinue} disabled={isSaving}>
              {isSaving ? <span className="button-spinner" aria-hidden="true" /> : <Icon name="check" />}
            ...
            <div className="today-card">
              <div className="live-earnings-panel">
                <div className="live-metric-card">
                  ...
                  <strong className={isWorkTimerRunning ? "animated-number" : ""}>
                    <RollingNumber value={activeWorkHoursText} active={isWorkTimerRunning} />
                  </strong>
                </div>
                <div className="live-metric-card">
                  <span>Earned so far</span>
                  <strong className={isWorkTimerRunning ? "animated-number money-number" : "money-number"}>
                    <RollingNumber value={activeWorkPayText} active={isWorkTimerRunning} />
                  </strong>
                </div>
              </div>
              ...
              <section
                className={`iphone-clock-card ${isWorkTimerRunning ? "is-running" : ""} ${currentWorkDayCompleted ? "is-complete" : ""}`}
                style={{
                  "--clock-progress": activeClockProgress,
                  "--clock-second-angle": `${clockSecondAngle}deg`,
                  "--clock-minute-angle": `${clockMinuteAngle}deg`,
                  "--clock-hour-angle": `${clockHourAngle}deg`
                }}
                aria-label="Live shift timer"
              >
                <div className="clock-face" aria-hidden="true">
                  <div className="clock-progress-ring" />
                  <div className="clock-dial">
                    <span className="clock-mark top">12</span>
                    <span className="clock-mark right">3</span>
                    <span className="clock-mark bottom">6</span>
                    <span className="clock-mark left">9</span>
                ...
                <div className="clock-copy">
                  <span>{isWorkTimerRunning ? "Live shift timer" : currentWorkDayCompleted ? "Today's shift saved" : "Tap start to punch in"}</span>
                </div>
              </section>
              ...
              <section className={`primary-punch-panel ${isWorkTimerRunning ? "is-running" : ""}`} aria-label="Shift control">
                <div>
                  ...
                  <strong>
                    {isWorkTimerRunning ? (
                      <RollingNumber value={formatRunningTime(currentWorkDay, activeTimer, now)} active />
                    ) : currentWorkDayCompleted ? "Done" : "Start when ready"}
                  </strong>
                </div>
                <button
                ...
                >
                  {isSaving ? <span className="button-spinner" aria-hidden="true" /> : <Icon name={isWorkTimerRunning || currentWorkDayCompleted ? "stop" : "play"} />}
                  {primaryShiftLabel}
                </button>
              </section>
              <div className="clock-stats shift-quick-details" aria-label="Current shift details">
                <small>Start <b>{formatDisplayTime(currentWorkDay.start, timeFormat)}</b></small>
                <small>End <b>{formatDisplayTime(currentWorkDay.end, timeFormat)}</b></small>
                <small>
                  Hours
                  <b>
                    <RollingNumber value={activeWorkHours.toFixed(isWorkTimerRunning ? 4 : 2)} active={isWorkTimerRunning} />
                  </b>
                </small>
              </div>
